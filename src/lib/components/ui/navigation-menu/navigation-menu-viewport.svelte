...
<div class={cn("absolute inset-s-0 top-full isolate z-50 flex justify-center")}>
	<NavigationMenuPrimitive.Viewport
		...
		class={cn(
			"bg-popover text-popover-foreground data-open:animate-in data-closed:animate-out data-closed:zoom-out-90 data-open:zoom-in-90 ring-foreground/10 rounded-lg shadow ring-1 duration-300 ease-[cubic-bezier(0.22,1,0.36,1)] origin-top-center relative mt-1.5 h-[calc(var(--bits-navigation-menu-viewport-height)+1rem)] w-full overflow-hidden md:w-[calc(var(--bits-navigation-menu-viewport-width)+1rem)]",
			className
		)}
		{...restProps}
	/>
